...
def trigger_crash_parallel(
    ...
    """
    Trigger crash in multiple VMs simultaneously using threads.

    Creates threads to trigger crashes in all VMs in parallel, waits for
    ...
    for vm in vms:
        ...
        threads.append(
            threading.Thread(
                target=trigger_crash,
                args=(vm, session, enable_sysrq_cmd, trigger_crash_cmd, wait_time, test),
            )
        )
    # Give all sessions a moment to settle before triggering in parallel
    time.sleep(20)
